...
# 全局字体设置，兜底用Linux自带的文泉驿字体
plt.rcParams['axes.unicode_minus'] = False
if my_font:
    plt.rcParams['font.sans-serif'] = [my_font.get_name(), 'WenQuanYi Micro Hei', 'SimHei']
    sns.set(font=my_font.get_name(), font_scale=1.1)
else:
    plt.rcParams['font.sans-serif'] = ['WenQuanYi Micro Hei', 'SimHei']
    sns.set(font='WenQuanYi Micro Hei', font_scale=1.1)
...
def LM_optimize_general(x, y, init_params, model_func, max_iter=100):
    def FX(w): return y - model_func(x, w)
    def JJ(w, h=1e-6):
        n = len(w)
        m = len(x)
        J = np.zeros((m, n))
...
if model_type == "通用曲线拟合（所有专业通用）":
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 一键高精度拟合（自动跑3次选最优）"):
            with st.spinner("正在自动拟合，跑3次选最好的结果，稍等..."):
                np.random.seed(42)
                ...
                ax1.set_title('高精度拟合结果', fontproperties=my_font)
                ax1.legend(prop=my_font)
                ax1.grid(alpha=0.3)
                ax1.set_xlabel('x', fontproperties=my_font)
                ax1.set_ylabel('y', fontproperties=my_font)
                sns.despine(ax=ax1)
                ...
                ax2.set_ylabel('损失', fontproperties=my_font)
                ax2.set_yscale('log')
                # 细粒度的log刻度，不会只显示10²
                ...
                ax2.set_title('DE+LM 联合优化收敛曲线', fontproperties=my_font)
...
# 分支2：混淆矩阵&分类报告
# --------------------------
elif model_type == "混淆矩阵&分类报告（模式识别实验）":
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 生成混淆矩阵&分类报告"):
            with st.spinner("正在计算..."):
                ...
                ax.set_xlabel("预测标签", fontproperties=my_font)
                ax.set_ylabel("真实标签", fontproperties=my_font)
                ax.set_title("混淆矩阵", fontproperties=my_font)
                sns.despine(ax=ax)
                st.pyplot(fig)
...
# 分支3：ROC&AUC计算
# --------------------------
elif model_type == "ROC曲线&AUC计算（模式识别实验）":
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 生成ROC曲线&AUC"):
            with st.spinner("正在计算..."):
                ...
                ax.set_xlabel("假正率 (FPR)", fontproperties=my_font)
                ax.set_ylabel("真正率 (TPR)", fontproperties=my_font)
                ax.set_title("ROC曲线", fontproperties=my_font)
                ax.legend(loc="lower right", framealpha=0.9, prop=my_font)
                sns.despine(ax=ax)
                st.pyplot(fig)
...
# 分支4：学习曲线自动拟合
# --------------------------
elif model_type == "学习曲线自动拟合（模式识别实验）":
    ...
    if uploaded_file is not None:
        try:
            df = pd.read_csv(uploaded_file, header=None, names=["train_size", "train_error", "test_error"], sep=None, engine='python')
            # 自动检测表头
            try:
                df.iloc[0].astype(float)
            except:
        ...
        if st.button("🚀 生成学习曲线"):
            with st.spinner("正在拟合，自动选最优结果..."):
                ...
                ax.set_xlabel("训练样本量", fontproperties=my_font)
                ax.set_ylabel("误差", fontproperties=my_font)
                ax.set_title("学习曲线", fontproperties=my_font)
                ax.legend(loc="best", framealpha=0.9, prop=my_font)
                sns.despine(ax=ax)
                st.pyplot(fig)

                # 自动提示
                if gap > 0.1:
                    st.warning(f"检测到过拟合：训练误差{train_final:.4f}，测试误差{test_final:.4f}，差距较大，建议增加数据或正则化")
...
# 分支5：数据标准化/归一化
# --------------------------
elif model_type == "数据标准化/归一化（数模预处理）":
...
# --------------------------
elif model_type == "✨ 自定义模型参数拟合（核心创新）":
    st.subheader("核心创新：自定义模型参数拟合")
    st.info("""
    ...
    default_formula = template_options[selected_template] if selected_template != "自定义输入" else "a * exp(-b * x) + c"
        ...
        if st.button("🚀 一键拟合参数（自动跑3次选最优）"):
            with st.spinner("正在DE全局搜索+LM精细优化，跑3次选最好的结果，稍等..."):
                try:
                    ...
                    ax.set_title(f'自定义模型拟合结果：{formula}', fontproperties=my_font)
                    ax.legend(framealpha=0.9, prop=my_font)
                    sns.despine(ax=ax)
                    st.pyplot(fig)
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 一键补全缺失数据（自动选最优）"):
            with st.spinner("正在DE+LM高精度拟合，自动选最好的结果，补全缺失点..."):
                ...
                ax.set_xlabel('x', fontproperties=my_font)
                ax.set_ylabel('y', fontproperties=my_font)
                ax.set_title('高精度插值补全结果', fontproperties=my_font)
                ax.legend(framealpha=0.9, prop=my_font)
                sns.despine(ax=ax)
                st.pyplot(fig)
...
# 分支8：无模型时间序列预测
# --------------------------
elif model_type == "无模型时间序列预测（数模专用）":
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 一键预测未来数据（自动选最优）"):
            with st.spinner("正在DE+LM学习数据规律，自动选最好的结果，预测未来点..."):
                ...
                ax.set_xlabel('时间/x', fontproperties=my_font)
                ax.set_ylabel('值/y', fontproperties=my_font)
                ax.set_title('无模型时间序列预测结果', fontproperties=my_font)
                ax.legend(framealpha=0.9, prop=my_font)
                sns.despine(ax=ax)
                st.pyplot(fig)
                ...
                result_df = pd.DataFrame({'x': x_all, 'y': y_all, '类型': ['历史']*len(x_raw) + ['预测']*len(x_future)})
                csv = result_df.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig')
                st.download_button(
                    "下载完整的预测数据", 
                    csv, 
